...
const EXAMPLE_DATA: SimpleTransaction[] = [
  { accountId: "A1", amount: 20000, timestamp: "2026-02-19T10:00:00Z", city: "Delhi" },
  { accountId: "A1", amount: 15000, timestamp: "2026-02-19T10:00:30Z", city: "Delhi" },
  { accountId: "A1", amount: 20000, timestamp: "2026-02-19T10:01:00Z", city: "Mumbai" },
];
...
const Index = () => {
  ...
  const parseJsonInput = (data: any): SimpleTransaction[] => {
    ...
    // Handle if JSON is an object with a nested array property
    if (!Array.isArray(data)) {
      if (typeof data === 'object' && data !== null) {
        // Try common property names for nested arrays
        arrayData = data.transactions || data.data || data.records || data.rows || [];
    ...
    return arrayData.map((d: any) => {
      ...
      const accountId = d.accountId || d.AccountID || d.account_id || d.account || d.Account || "UNKNOWN";
      const amountStr = d.amount || d.TransactionAmount || d.transaction_amount || d.Txn_Amt || d.Amount || "0";
      const amount = parseFloat(String(amountStr)) || 0;
      const timestamp = String(d.timestamp || d.TransactionDate || d.transaction_date || d.date || d.Date || d.Txn_Date || d.Time || "").trim();
      const city = String(d.city || d.City || d.Location || d.location || d.Place || d.place || "Unknown").trim();
    ...
    }).filter(t => t.accountId !== "UNKNOWN" && t.amount > 0);
  };
    ...
    reader.onload = (ev) => {
      try {
        ...
        try {
          ...
          // parseJsonInput handles both arrays and objects with nested arrays
          const parsed = parseJsonInput(data);
          if (parsed.length > 0) {
            console.log("Loaded from JSON:", parsed.length, "transactions");
          ...
          console.log("Not valid JSON or no valid transactions found, trying CSV format...", jsonError);
        }
        
        // Fall back to CSV parsing only if JSON didn't successfully parse
        if (!jsonParsed) {
          try {
            const parsed = parseCSV(text);
            if (parsed.length > 0) {
              console.log("Loaded from CSV:", parsed.length, "transactions");
              processData(parsed);
            } else {
              console.warn("No valid data found in file");
              alert("No valid transactions found in file. Please check the format.");
            }
          } catch (csvError) {
            console.error("CSV parsing error:", csvError);
            alert("Error parsing file. Please ensure it's valid CSV or JSON.");
          }
        }
      } catch (error) {
